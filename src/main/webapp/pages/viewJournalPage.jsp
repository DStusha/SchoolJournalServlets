...
<table border=1>
    ...
    <tbody>
        ...
        <tr>
            ...
            %>
            <td><a href="view?action=homework&class=<c:out value="${cl}" />&week=<c:out value="${day}" />&subjId=<c:out value="${sday.id}"/>&value=<c:out value="${sday.homework}"/>">Изменить домашнее задание</a><br></td>
            <%            } else {
            %>
            <td><a href="view?action=mark&class=<c:out value="${cl}" />&week=<c:out value="${day}" />&subjId=<c:out value="${sday.id}"/>&value=<c:out value="${sday.mark}"/>">Изменить оценку</a></td>
            <%
                }
            %>
        </tr>
